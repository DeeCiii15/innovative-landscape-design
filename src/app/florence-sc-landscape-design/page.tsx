...
import { getProjectCover, getProjectsByService } from "@/lib/loadProjects";
import { getMainService } from "@/lib/servicesData";
import { getProcessStep } from "@/lib/processData";
import { workItemPath } from "@/lib/workData";
import { siteConfig } from "@/lib/siteConfig";

export const metadata: Metadata = {
  title: { absolute: FLORENCE_LANDSCAPE_TITLE },
  description: `Landscape design in Florence, ${PRIMARY_STATE_ABBR} from ${siteConfig.name}. We plan, install, and maintain landscapes for homeowners across the Florence area.`,
...
export default async function FlorenceLandscapeHub({ searchParams }: LocationHubProps) {
  ...
  const design = getProcessStep("design");
  const featuredWork = getProjectsByService(service.slug).slice(0, 3);
  const backHref =
  ...
  return (
    <InnerPage
      hero={
        <PageHero
          title="Florence, SC Landscape"
          accent="Design"
          image={design?.galleryImage ?? service.galleryImage}
          imageAlt={design?.galleryImageAlt ?? service.galleryImageAlt}
        />
      }
    >
      <section className="py-16 sm:py-24 bg-[var(--color-canvas)]">
        <div className="container-main max-w-3xl">
          ...
          <p className="lead mt-5">
            Custom landscape design for homeowners in Florence and nearby communities including{" "}
            {SERVICE_AREAS.filter((city) => city !== "Florence").join(", ")}.
          </p>
          <p className="lead mt-4">{design?.intro ?? service.intro}</p>
          <p className="lead mt-4">{design?.body ?? service.body}</p>
          <div className="mt-8 flex flex-wrap gap-4">
            <Link href="/contact" className="btn-primary">
              Request a free estimate
            </Link>
            <Link href="/#process" className="btn-secondary">
              How landscape design works
            </Link>
          </div>
        </div>
      </section>
            ...
            {featuredWork.map((item) => {
              ...
              >
                ...
                <div className="px-5 py-5">
                  <h3 className="font-medium tracking-tight text-[var(--color-ink)]">{item.title}</h3>
